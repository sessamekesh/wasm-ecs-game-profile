...
async function tryOpenFile(filePath) {
    // This is a silly little thing to help out with debug symbols
    //  FOR THIS PROJECT (wasm-ecs-game-profile) SPECIFICALLY.
    // If the file isn't found in the output binary directory, look for it
    //  instead up at the project root.
    // Assuming the output is at out/wasmbuild/igdemo.js, this works!
    const pathRoots = ['.', '../../'];
    for (let i = 0; i < pathRoots.length; i++) {
        const path = pathRoots[i] + '/' + filePath;
        try {
...
const port = process.argv.filter(arg => arg.startsWith("--port=")).map(arg => arg.substring("--port".length))?.[0] || 8000;
...
http.createServer(async (request, response) => {
    ...
    try {
        const [stat, realPath] = await tryOpenFile(filePath);
        if (stat.isFile()) {
